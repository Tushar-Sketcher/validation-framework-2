...
def compare_counts(table1_df, table2_df):
    """
    Performs count validations on dataframes based on configuration parameters provided in yaml file as input and
    returns a DataFrame. The function iterates through the rows in the provided configuration and performs count
    validations on the specified tables.
...
def compare_rows(table1_df_ordered, table2_df_ordered):
    """
    Perform row-level validation between two tables using our MD5 hashing approach.
    Params:
    - table1_df_ordered (dataframe): First table dataframe.
    - table2_df_ordered (dataframe): Second table dataframe.
    Returns:
    - table1_subtract_cnt, table2_subtract_cnt, table1_datamatch_st, table2_datamatch_st: variables contain the row validation details.
    """
    table1_df_ordered = table1_df_ordered.withColumn("checksum", md5(concat_ws("", *[get_coalesce(table1_df_ordered, col_val)
                                                                     for col_val in table1_df_ordered.columns])))
    table2_df_ordered = table2_df_ordered.withColumn("checksum", md5(concat_ws("", *[get_coalesce(table2_df_ordered, col_val)
                                                                     for col_val in table2_df_ordered.columns])))
    table1_md5 = table1_df_ordered.select("checksum").distinct().orderBy("checksum")
    table2_md5 = table2_df_ordered.select("checksum").distinct().orderBy("checksum")
...
def compare_metrics_by_dimension(table1, table2, query_filter="", dimension_cols="", dim_metrics_cols="",
                                 ignored_cols="", metric_result_table="", spark=""):
    """
    ...
    - dimension_cols (str): List of dimension columns with comma seperated values to include in the validation.
    ...
    """
    dimension_cols = [] if dimension_cols == "" else list(dimension_cols.split(', '))
    ...
    tb1_metric_cols = sorted([col_name.lower() for col_name, data_type in tb1.dtypes if
                              data_type.startswith(('bigint', 'int', 'double', 'float', 'decimal'))])
    tb2_metric_cols = sorted([col_name.lower() for col_name, data_type in tb2.dtypes if
                              data_type.startswith(('bigint', 'int', 'double', 'float', 'decimal'))])
    ...
    try:
        if not tb1_metric_cols and not tb2_metric_cols:
            raise ValidationError(
                f"ERROR ::: Neither {table1} nor {table2} contains any metric columns of type 'bigint','int','double' or 'float'. Validation SQL query generation cannot proceed for these tables.")
        elif tb1_metric_cols != tb2_metric_cols:
            raise ValidationError(
                f"ERROR ::: The metric columns retrieved for {table1} and {table2} are not identical. Please verify both table's columns before proceeding.\n{table1} No of metrics: {len(tb1_metric_cols)} columns: {tb1_metric_cols}\n{table2} No of metrics: {len(tb2_metric_cols)} columns: {tb2_metric_cols}")
        else:
            ...
            dim_metrics_diff_final = [(
                f"{col}_tb1_cnt",
                f"{col}_tb2_cnt",
                f"ROUND(COALESCE({col}_tb1_cnt, 0) - COALESCE({col}_tb2_cnt, 0), 4) AS {col}_cnt_diff",
                f"ROUND(((COALESCE({col}_tb1_cnt, 0) - COALESCE({col}_tb2_cnt, 0))/COALESCE({col}_tb1_cnt, 0))*100, 4) AS cnt_per_diff_{col}"
            ...
            join_cols = [f"table1.{col} = table2.{col}" for col in dimension_cols]
            ...
            sql = """
                    ...
                    SELECT * FROM final""".format(table1=table1,
                                                  table2=table2,
                                                  cols1=", ".join(dimension_cols + metric_dim_agg_tb1 + sum_agg_tb1),
                                                  cols2=", ".join(dimension_cols + metric_dim_agg_tb2 + sum_agg_tb2),
                                                  where=' WHERE ' + query_filter if query_filter else '',
                                                  groupby=' GROUP BY ' + ", ".join(
                                                      dimension_cols) + ' ORDER BY ' + ", ".join(
                                                      dimension_cols) + ' DESC' if dimension_cols else '',
                                                  dim_cols=" ".join(
                                                      [f"COALESCE(table1.{col}, table2.{col}) AS {col}," for col in
                                                       dimension_cols]),
                                                  dim_metrics_diff_final=", ".join(
                                                      [", ".join(diff_cols) for diff_cols in
                                                       dim_metrics_diff_final]) + ',' if dim_metrics_diff_final else '',
                                                  metric_diff_final=", ".join(
                                                      [", ".join(diff_col) for diff_col in metric_diff_final]),
            ...
            print(
                f'List Of Metric Columns Compared From {table1} and {table2} Table: {dim_metrics_cols + tb1_metric_cols}\n')
            print(f'List Of Dimension Columns Added From the tables: {dimension_cols}\n')
            ...
            try:
                ...
                print(
                    f"Successfully stored the metric validation data in table: {metric_result_table}. Please query this table to check validation details.")
                return metric_df
...
def compare_two_tables(spark, table1, table2, query_filter, metric_validation_active, dimension_cols, dim_metrics_cols,
                       ignored_cols, metric_result_table, materialization, result_rows=None):
    """
    ...
    if result_rows is None:
        result_rows = []
    try:
        table1_query = get_query(table1, query_filter)
        table2_query = get_query(table2, query_filter)
        ...
        if is_column_name_match:
            ...
            result_rows.append((table1, table2, query_filter, table1_count, table2_count, diff, diff_per, count_match_status,
                                table1_subtract_count,
                                table2_subtract_count, partition_status_table1, partition_status_table2,
                                partition_columns_table1, partition_columns_table2, materialization))

            if metric_validation_active:
                if metric_result_table == '':
                    metric_result_table = '{catalog}.{schema}.{table_name}_metric_validation'.format(
                        catalog='sandbox_marketing', schema='mart_mops_test', table_name=table2.split('.')[2])
                compare_metrics_by_dimension(table1, table2, query_filter, dimension_cols, dim_metrics_cols, ignored_cols,
                                             metric_result_table, spark)

        else:
            print(
                f"- The Row Data Validation process skipped, as the Columns of the tables {table1} and {table2} are not identical. Please verify the columns of the tables.")
            ...
            print(
                f"- Row Data validation related columns will be set to NULL in the final result set for these tables.\n")
